...
const insertMovieGenres = (
  ...
  var genre = genres.map(
    genre => 
    { 
      var currentRow = genreRows.find(row => row.genre == genre);     
      return `(${movieId}, ${currentRow ? currentRow.id : 0})`;
    }
  );
  return (
    `INSERT INTO ${MOVIE_GENRES} (movie_id, genre_id) VALUES` + genre.join(",")
  )
};
...
const insertMovieActors = (
  ...
  var actor = actors.map(
    actor => 
    { 
      var currentRow = actorRows.find(row => row.full_name == actor);     
      return `(${movieId}, ${currentRow ? currentRow.id : 0})`;
    }
  );
  return (
    `INSERT INTO ${MOVIE_ACTORS} (movie_id, actor_id) VALUES` + actor.join(",")
  )
};
...
const insertMovieDirectors = (
  ...
  var director = directors.map(
    director => 
    { 
      var currentRow = directorRows.find(row => row.full_name == director);     
      return `(${movieId}, ${currentRow ? currentRow.id : 0})`;
    }
  );
  return (
    `INSERT INTO ${MOVIE_DIRECTORS} (movie_id, director_id) VALUES` + director.join(",")
  )
};
...
const insertMovieKeywords = (
  ...
  var keyword = keywords.map(
    keyword => 
    { 
      var currentRow = keywordRows.find(row => row.keyword == keyword);     
      return `(${movieId}, ${currentRow ? currentRow.id : 0})`;
    }
  );
  return (
    `INSERT INTO ${MOVIE_KEYWORDS} (movie_id, keyword_id) VALUES` + keyword.join(",")
  )
};
...
const insertMovieProductionCompanies = (
  ...
  var company = productionCompanies.map(
    company => 
    { 
      var currentRow = productionCompanyRows.find(row => row.company_name == company);     
      return `(${movieId}, ${currentRow ? currentRow.id : 0})`;
    }
  );
  return (
    `INSERT INTO ${MOVIE_PRODUCTION_COMPANIES} (movie_id, company_id) VALUES` + company.join(",")
  )
};
